-- Teacher Portal live-data diagnostic.
-- Replace the email below when diagnosing another teacher. This file only reads data.
with params as (
...
students_missing_class_id_matching_class_section as (
  select
    st.*,
    c.id as expected_class_id,
    coalesce(c.class_name, c.name, c.grade) as expected_class_name,
    c.section as expected_section
  from public.students st
  join assigned_classes c
    on (
      lower(trim(coalesce(st.class_name, ''))) = lower(trim(coalesce(c.class_name, c.name, c.grade, '')))
      or lower(trim(coalesce(st.class_name, ''))) = lower(trim(concat_ws('-', nullif(coalesce(c.class_name, c.name, c.grade), ''), nullif(c.section, ''))))
    )
   and (
...
union all
select '05_students_by_class_id', jsonb_agg(to_jsonb(st))
...
union all
select '08_students_missing_class_id_matching_class_section', jsonb_agg(to_jsonb(st))
from students_missing_class_id_matching_class_section st;
